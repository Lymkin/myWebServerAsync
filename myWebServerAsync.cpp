...
Version 2.0 by Rodney Adams (rodney.adams.69@gmail.com). All rights reserved.
...
//Arduino Json here:  https://github.com/bblanchon/ArduinoJson <---Tested on version 5.13.5 (version 6.0+ breaks json function)
//Async Web Server https://github.com/me-no-dev/ESPAsyncWebServer
...
{
	DebugPrintln("isAdmin request from - " + request->client()->remoteIP().toString());
...
void handleFileList(AsyncWebServerRequest *request)
{
	...
	while (dir.next()) {
		if (!firstrec) {
			output += ',';
		}  //add comma between recs (not first rec)
		else {
			firstrec = false;
		}
...
void handleReadJson(AsyncWebServerRequest *request)
{
	if (isAdmin(request) == false) return;

	if (request->arg(0U) == 0)
		return request->send(500, "application/json", "{json: \"no arg\"}");

	String pName = ""; String pValue = ""; String pJson = "";
	AsyncWebParameter* p = request->getParam(0);
	pName = p->name();
	pValue = "/" + p->value();

	if (pName == "read") {
		File file = SPIFFS.open(pValue, "r");
		if (file) {
			file.close();
			AsyncWebServerResponse *response = request->beginResponse(SPIFFS, pValue);
			response->addHeader("Access-Control-Allow-Origin", "*");
			request->send(response);
			DebugPrintln("handleReadJson request for : " + pValue);
		}
		else {
			request->send(500, "application/json", "{json: \"bad value\"}");
		}
	}
	else {
		request->send(500, "application/json", "{json: \"bad key\"}");
	}
}
...
{
	if (isAdmin(request) == false) return;
	DebugPrintln("Formatting SPIFFS.....");
	if (!SPIFFS.format()) {
...
{
	DebugPrintln("Starting ESP8266 AsyncWebServer");

	bool result = SPIFFS.begin();
	DebugPrint("SPIFFS opened: "); DebugPrintln(result ? "OK" : "FAIL");
	...
	else if (MDNSdisable == false) {
		MDNS.begin(DeviceName.c_str());  //multicast webname when not in SoftAP mode
		DebugPrintln("Starting mDNS " + DeviceName + ".local");
		MDNS.addService("http", "tcp", 80);
	}

// Server Routes

	AsyncCallbackJsonWebHandler* handler = new AsyncCallbackJsonWebHandler("/savejson", [](AsyncWebServerRequest *request, JsonVariant &json) {
	  ...
	  pName = p->name();
		...
		if (pName == "save") {
			jsonObj.prettyPrintTo(pJson);
			File file = SPIFFS.open(pValue, "w");
			if (file) {
				file.println(pJson);  //save json data	
				file.close();
				request->send(200, "application/json", "{save: \"ok\"}");
			}
			else {
	  		request->send(500, "application/json", "{json: \"bad value\"}");
			}
	  	DebugPrintln("JSON = " + pJson);
		}
		else {
			request->send(500, "application/json", "{json: \"bad key\"}");
		}
	...
	server.on("/readjson", handleReadJson);
	server.on("/serverlog", SendServerLog);
	server.on("/generate_204", handleRoot);  //use index.html or use embedded wifi setup...);
	...
	DebugPrintln("ESP8266 AsyncWebServer Startup Complete");
	...
	if (StartAP) {
	  ...
	  DebugPrintln("Starting Scan for WiFi Networks......");
	}
...
//		JsonObject& root = jsonBuffer.parseObject(values);  //parse weburl
		...
		if (String(root["ssid"].asString()) != "") { //verify good json info                                                
			...
			DebugPrintln("WiFi config read successfully");
			return true;
		}
	} //file exists;
	DebugPrintln("WiFi config has bad data");
